...
class GroupSummary(Node):
	...
	def infer_output_info(self, inputs):
		...
		elif isinstance(self.aggr_col, list):
	...
	def eval(self, inputs):
		...
		if self.group_cols:
			...
			for (key, group) in grouped_res:
				...
				elif self.aggr_func == "rownum":  # allow any column to be target col
					for row_index in index_list:
						# arguments[row_index][new_col] = [(cid, rid) for cid in range(len(df.columns)) for rid in index_list]
						arguments[row_index][new_col] = [(self.target_col, rid) for rid in index_list]
				else:
			...
			if self.aggr_func == "cumsum":
				...
				if self.aggr_func == "rownum":
					...
					for index in arguments:
						arguments[index][new_col] = [(self.target_col, rid) for rid in arguments]
				else:
