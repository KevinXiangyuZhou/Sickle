...
from table_ast import (HOLE, Node, Table, Select, Unite, Filter, GroupSummary, GroupMutate, CumSum, Mutate)
abstract_combinators = {
    ...
    "group_mutate": lambda q: GroupMutate(q, group_cols=HOLE, target_col=HOLE, aggr_func=HOLE),
    "cumsum": lambda q: CumSum(q, target=HOLE),
    "mutate": lambda q: Mutate(q, target_col=HOLE, func=HOLE, agg=HOLE)
}
...
class Synthesizer(object):
    def __init__(self, config=None):
        if config is None:
            self.config = {
                "operators": ["select", "unite", "filter", "group_sum", "group_mutate", "cumsum", "mutate"],
                "filer_op": [">", "<", "=="],
                "constants": [],
            ...
            if p.is_abstract():
                ...
                for _ast in next_level_programs:
                    ...
                    for premise, subquery_path in premises_at_level:
                        ...
                        # print(subquery_res)
                        # print(subquery_res)
                        if check_table_inclusion(premise.to_dict(orient="records"),
                                                 subquery_res.to_dict(orient="records")):
                            # debug
